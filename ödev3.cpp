...
long long recursiveFunction(int numberOfStones)
{
	...
	default:
		return recursiveFunction(numberOfStones - 1) + recursiveFunction(numberOfStones - 2) + recursiveFunction(numberOfStones - 3); //bu kısım memoization kısmıdır sayılar he defasında 1-2-3 adımdarındaki yollara bölünür.
	}
}
...
long long iterativeFunction(int numberOfStones,int i)
{
	long long yolsayisi;
	vector <long long> zipzip;  // 72 adımın herbirinde kaç yol olduğunu tutan vector
	...
	{
		zipzip.push_back(toplam);
		yolsayisi = zipzip[i-1] ;
