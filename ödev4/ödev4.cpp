...
void fillTriangles(vector<Triangle>&v, int numberOfItems = 100)
{
	for (size_t i{ 0 }; i < numberOfItems; ++i)
	{
		...
		int e = dist(mt);
		int f = dist(mt);
		v.push_back(Triangle{ Point(a,b),Point(c,d),Point(e,f)});
...
void fillQuads(vector<Quadrilateral>& v, int numberOfItems = 100)
{
	for (size_t i{ 0 }; i < numberOfItems; ++i)
	{
		...
		int a = randomint(engine);
		int b = randomint(engine);
		int c = randomint(engine);
		int d = randomint(engine);
		int e = randomint(engine);
		int f = randomint(engine);
		int g = randomint(engine);
		int h = randomint(engine);

		v.push_back( Quadrilateral (Point(a,b),Point(c,d),Point(e,f),Point(g,h)));
